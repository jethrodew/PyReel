...
def print_weather(obs,fore):
	print('\n')
	print('- '*20)
	delay_print("Current Temperature (Oxford)")
	print('- '*10)
	delay_print(obs.entries[0].title.replace('°',''))
	print('\n')
	print('- '*20)
	delay_print("3 Day Forecast (Oxford)")
	print('- '*10)
...
# Run Functions
def bbc_news():
	f = feedparser.parse('http://feeds.bbci.co.uk/news/rss.xml?edition=uk') #BBC News Frontpage
	bbc_news_logo()
...
def bbc_technology_news():
	f = feedparser.parse('http://feeds.bbci.co.uk/news/technology/rss.xml?edition=uk#') #BBC Technology News
	bbc_logo()
	technology_logo()
	for post in f.entries[:10]:
		print_rss(post)
...
def bbc_weather():
	w = feedparser.parse('http://open.live.bbc.co.uk/weather/feeds/en/2640729/3dayforecast.rss') #BBC Weather 3 Day Forecast (Oxford)
	o = feedparser.parse('http://open.live.bbc.co.uk/weather/feeds/en/2640729/observations.rss') #BBC Weather Observations (Oxford)
	bbc_logo()
	weather_logo()
	print_weather(o,w)
	delay(2)
